...
// Language names for mock translations
...
// Mock translation function - adds language prefix for testing
function mockTranslate(text, targetLang) {
  // This creates a mock translation by adding the language code as a prefix
  // In production, this would be replaced with actual API calls
  return `[${targetLang.toUpperCase()}] ${text}`;
}

// Mock HTML translation - preserves structure
function mockTranslateHTML(html, targetLang) {
...
async function generateMockTranslations() {
  ...
  console.log('NOTE: These are MOCK translations. Replace with real Google Translate API calls.\n');
  ...
  // Add header comments
  sqlStatements.push('-- IMPORTANT: These are MOCK translations for testing purposes');
  sqlStatements.push('-- Replace [IT], [ES], [HU] prefixes with actual translations');
  sqlStatements.push('-- Use the translate-database-content.js script with a valid API key\n');
  ...
  for (const post of blogPosts) {
    ...
    for (const lang of TARGET_LANGUAGES) {
      translations[lang] = {
        title: mockTranslate(post.title, lang),
        excerpt: mockTranslate(post.excerpt, lang),
        content: mockTranslateHTML(post.content, lang)
      };
  ...
  for (const resource of resources) {
    ...
    for (const lang of TARGET_LANGUAGES) {
      translations[lang] = {
        title: mockTranslate(resource.title, lang),
        description: mockTranslate(resource.description, lang),
        content: mockTranslateHTML(resource.content, lang)
      };
  ...
  for (const project of projects) {
    ...
    for (const lang of TARGET_LANGUAGES) {
      translations[lang] = {
        title: mockTranslate(project.title, lang),
        description: mockTranslate(project.description, lang)
      };
  ...
  // Write SQL file
  const outputPath = path.join(__dirname, '..', 'database-translations-mock.sql');
  const sqlContent = '-- Mock database translations generated by translate-database-content-mock.js\n' +
  ...
  console.log(`Total items processed: ${blogPosts.length + resources.length + projects.length}`);
  console.log(`Total characters that would be translated: ${totalCharacters * 3} (across 3 languages)`);
  console.log(`Estimated Google Translate API cost: $${estimatedCost.toFixed(4)}`);
  console.log(`\nIMPORTANT: This generated MOCK translations. To get real translations:`);
  console.log(`1. Get a valid Google Translate API key`);
  console.log(`2. Update the API_KEY in translate-database-content.js`);
  console.log(`3. Run: node scripts/translate-database-content.js`);
}

// Run the mock translation
generateMockTranslations().catch(error => {
